...
const VirtualTryOn = () => {
  ...
  // State quản lý đường dẫn áo
  const [clothPath, setClothPath] = useState('./ao3.png');
  const [customPath, setCustomPath] = useState('');

  // State lưu danh sách sản phẩm từ Shop
  const [shopProducts, setShopProducts] = useState([]);

  // 🆕 State lưu danh sách sản phẩm từ Cart
  const [cartItems, setCartItems] = useState([]);
  const [activeTab, setActiveTab] = useState('shop'); // 'shop' hoặc 'cart'

  // 1. LẤY DANH SÁCH SẢN PHẨM TỪ SHOP VÀ CART KHI VÀO TRANG
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (cartData && cartData.items) {
          // Normalize cart items để dễ xử lý
    ...
    // Khởi tạo backend với áo mặc định
    initializeBackend(clothPath);
  }, []);

  // 2. LOGIC KHỞI TẠO BACKEND
  ...
  // 3. XỬ LÝ KHI CHỌN SẢN PHẨM TỪ SHOP
  ...
  // 🆕 Render danh sách sản phẩm dựa vào tab đang active
    ...
    return (
      <div className="quick-select-grid" style={{ gridTemplateColumns: 'repeat(3, 1fr)', gap: '12px' }}>
        {products.map((item) => {
          ...
          if (activeTab === 'cart') {
            // Xử lý cart item (đã được normalize)
            itemKey = item.id;
              ...
              <div style={{
                ...
                <img
                  ...
                  onError={(e) => {
                    ...
                    // Nếu ảnh trong /Product/ lỗi, thử /CustomProduct/
      ...
      <div className="virtual-tryon-wrapper">
        ...
        <div className="main-grid">
          ...
          <div className="settings-container">
            ...
            {/* 🆕 PHẦN CHỌN SẢN PHẨM VỚI TAB SHOP/CART */}
            <div className="panel">
