// src/pages/client/OrderHistory/OrderHistory.jsx
...
const OrderHistory = () => {
  ...
  // Primary batch fetch (tries batch endpoint, then per-id). returns map keyed by both string and number
  const fetchProductItemsByIds = async (ids) => {
    ...
    // Try batch endpoint
    try {
      // an toàn với các id có ký tự lạ
  ...
  // Additional fallback: try to fetch product name by many possible product endpoints (used when name still missing)
      ...
      try {
        ...
        for (const ep of endpoints) {
          try {
            ...
            // attempt to extract a human name from body
            const candidates = [
            ...
            // Sometimes API returns nested item: { item: { ... } }
    ...
    for (const name of candidates) {
      ...
      if (typeof fn === 'function') {
        try {
          ...
          try { return await fn.call(OrderHistoryService, { orderId }); } catch (_) { /* continue */ }
        }
      }
    }
  ...
  useEffect(() => {
    const fetchAll = async () => {
      try {
        ...
        // collect unique productItemIds (try many possible fields)
        const idSet = new Set();
        data.forEach(o => {
        ...
        // initial productItemMap attempt
        const productItemMap = await fetchProductItemsByIds(productItemIds);

        // normalize orders using productItemMap to pick product names if available
        let normalized = data.map(rawOrder => {
          try {
            ...
            const items = itemsRaw.map((it, idx) => {
              ...
              if (!foundName) {
                // log once for debugging; show raw item for the first occurrence to help mapping
        ...
        // --- After setting orders, detect productNames that are still fallback and try extra lookups ---
        // Collect missing productItemIds where productName is still "Sản phẩm #<id>"
