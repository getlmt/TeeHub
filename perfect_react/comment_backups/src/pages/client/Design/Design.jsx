...
// ⚠️ Đảm bảo bạn đã tạo file design.module.css cùng thư mục (như các bước trước)
...
// --- HELPER: TỰ CẬP NHẬT GIỎ HÀNG (BROADCAST) ---
function broadcastCartChange() {
    try {
        window.dispatchEvent(new Event('cartUpdated')); // Sự kiện 1
        window.dispatchEvent(new Event('cart-updated')); // Sự kiện 2 (dự phòng)
    } catch (e) { /* ignore */ }
    try {
        localStorage.setItem('cart', String(Date.now())); // Trigger storage event
    } catch (e) { /* ignore */ }
}
...
const TShirtDesigner = () => {
    ...
    // ✅✅✅ SỬA LỖI: PAYLOAD "BẤT TỬ" & BROADCAST EVENT
    const handleAddToCart = async (design) => {
        ...
        try {
            // PAYLOAD CHUẨN (Gửi kèm cả snake_case và camelCase để tránh lỗi backend)
            const cartPayload = {
                userId: currentUserId,
                qty: 1,
                
                // Key quan trọng cho Custom Product
                isCustomed: true, is_customed: true,
                customProductId: design.id, custom_product_id: design.id,
                
                // Key quan trọng cho Regular Product (gửi null để backend không bị lẫn)
            ...
            // 🔥 QUAN TRỌNG: Bắn sự kiện để Header tự cập nhật số lượng
            broadcastCartChange();

            // Cho người dùng chọn đi tiếp hay ở lại
